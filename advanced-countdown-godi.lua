...
function set_time_text(ns, text)
	...
	if string.match(text, "%%H") then
		local hours_infinite = math.floor(ms / 3600000)
		minutes_infinite = string.format("%2d", hours_infinite)
	...
	if string.match(text, "%%M") then
		local minutes_infinite = math.floor(ms / 60000)
		minutes_infinite = string.format("%2d", minutes_infinite)
	...
	if string.match(text, "%%S") then
		local seconds_infinite = math.floor(ms / 1000)
		seconds_infinite = string.format("%0d", seconds_infinite)
	...
	if string.match(text, "%%h") then
		local hours = math.floor(ms / 3600000) % 24
		hours = string.format("%2d", hours)
		text = string.gsub(text, "%%h", hours)
	end
	
	if string.match(text, "%%fh") then
		local hours = math.floor(ms / 3600000) % 24
		if hours == 1 then
			hours = string.format("%d", hours) .. " hour"
		else
			hours = string.format("%d", hours) .. " hours"
		end

		text = string.gsub(text, "%%fh", hours)
	end
	...
	if string.match(text, "%%m") then
		local minutes = math.floor(ms / 60000) % 60
		minutes = string.format("%2d", minutes)
	...
	if string.match(text, "%%s") then
		local seconds = math.floor(ms / 1000) % 60
		seconds = string.format("%2d", seconds)
	...
	if string.match(text, "%%dd") then
		local days = math.floor(ms / 86400000)
		days = string.format("%02d", days)
		text = string.gsub(text, "%%dd", tostring(days))
	end
	...
	if string.match(text, "%%fd") then
		local days = math.floor(ms / 86400000)
		if days == 1 then
			days = string.format("%d", days) .. " day"
		else
			days = string.format("%d", days) .. " days"
		end

		text = string.gsub(text, "%%fd", days)
	end
...
function activate_scene(next_scene)
	local current_scene = obs.obs_frontend_get_current_scene()
	local current_scene_name = obs.obs_source_get_name(current_scene)
	local scene = obs.obs_get_source_by_name(next_scene)

	if next_scene ~= current_scene_name then
		obs.obs_source_release(current_scene)
		obs.obs_frontend_set_current_scene(scene)
		-- obs.script_log(obs.LOG_INFO, "scene changed")
	else
		obs.script_log(obs.LOG_INFO, "already on the scene")
	end
end
...
function on_pause(pressed)
	...
	if timer_active then
		...
		stop_timer()
		if mode == "Normale GoDi" or mode == "Specific date and time" or mode == "Specific time" then
			reset(true)
		end
		start_timer()
	end
...
function on_godi(pressed, godihours, godiminutes, godiseconds)
	if not pressed then
		return
	end
	...
	obs.obs_data_set_int(settings, "hour", godihours)
	obs.obs_data_set_int(settings, "minutes", godiminutes)
	obs.obs_data_set_int(settings, "seconds", godiseconds)
...
function ninethirty_button_clicked(props, p)
	godihours = 9
	godiminutes = 30
	godiseconds = 0
	on_godi(true, godihours, godiminutes, godiseconds)
	return true
end

function tenthirty_button_clicked(props, p)
	godihours = 10
	godiminutes = 30
	godiseconds = 0
	on_godi(true, godihours, godiminutes, godiseconds)
...
function settings_modified(props, prop, settings)
	...
	local button_ninethirty = obs.obs_properties_get(props, "ninethirty_button")
	local button_tenthirty = obs.obs_properties_get(props, "tenthirty_button")
	local up_finished = obs.obs_properties_get(props, "countup_countdown_finished")

	if (mode_setting == "Normale GoDi") then
		...
		obs.obs_property_set_visible(button_ninethirty, true)
		obs.obs_property_set_visible(button_tenthirty, true)
	...
	elseif (mode_setting == "Countdown") then
		...
		obs.obs_property_set_visible(button_ninethirty, false)
		obs.obs_property_set_visible(button_tenthirty, false)
...
function script_properties()
	...
	obs.obs_property_list_add_string(p_mode, "Normale GoDi", "normal_service")
	...
	obs.obs_property_set_long_description(p, "Choose the text source you want to get updated by the timer script")
	...
	obs.obs_property_set_long_description(t, "Choose the scene you want to switch to after the timer has ended")
	obs.obs_property_list_add_string(t, CURRENT_SCENE_OPTION, CURRENT_SCENE_OPTION)
	...
	obs.obs_properties_add_button(props, "ninethirty_button", "09:30", ninethirty_button_clicked)
	obs.obs_properties_add_button(props, "tenthirty_button", "10:30", tenthirty_button_clicked)
...
function script_update(settings)
	...
	elseif mode == "Normale GoDi" then
		cur_time = delta_time(-1, -1, -1, godihours, godiminutes, godiseconds)
	else
		cur_time = 0
	end
...
function script_defaults(settings)
	...
	obs.obs_data_set_default_string(settings, "mode", "Normale GoDi")
